...
useEffect(()=>{
    ...
    return (
        ...
        <div className="weather">
            <h2 className="city"  id="root">City name :{aRname}</h2>
            <div className="temp" id="temp"> Temperature :{Math.floor(weather.data?.main.temp-273.15)}</div>
            <div className="description" id="description">{weather.data?.description}</div> 
            <div className="humidity" id="humidity">Humidity: {weather.data?.main.humidity} %</div>
            <div className="wind" id="wind">Wind Speed: {Math.floor(weather.data?.wind.speed*3.6)} Km/h</div>
        </div>
    </div>
    )
}
